...
let serviceAlertAudioUnlocked = false;
...
async function registerPushNotifications() {
  ...
  if (
    ...
    console.log("Este navegador no permite notificaciones push.");
...
async function submitTowQuote(currentRequest) {
  ...
  const tipoGruaText = window.prompt("Tipo de grúa que enviarás (Plataforma / Rescate / Mediana / Tráiler):", currentRequest.grua?.categoria || "Plataforma");

  if (tipoGruaText === null) return;

  const tipoGrua = String(tipoGruaText).trim();

  if (!tipoGrua) { toast("Ingresa el tipo de grúa que enviarás."); return; }
  ...
  await setDoc(quoteRef, {
    ...
    categoriaGrua: tipoGrua,

    distanciaKm: Number.isFinite(Number(currentRequest.__distanceKm)) ? Number(Number(currentRequest.__distanceKm).toFixed(2)) : null,
